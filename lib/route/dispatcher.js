...
function dispatchController(controller, container, method) {
    ...
    if (method_exists($class, method)) {
        ...
        if (vw instanceof Promise) {
            vw.then(CacheSystem => {
                for (let i = 0, len = CacheSystem.parsed_element.length, a = 0; i < len; i++) {
                    container.view.elm_container.appendChild(CacheSystem.parsed_element[a]);
                }
            });
