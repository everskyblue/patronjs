...
export class View {
    ...
    async getContent(file) {
        ...
        if (this.is_caching)
            this.hv.addCache(file, _document.body.innerHTML);
     ...
     */
    render(file, data) {
        this.setData(data).getContent(file).then(content => {
            document.querySelector(this.id_element).innerHTML = content;
        });
    }
    ...
    innerContent(content) {
        (this.elm_container || document.body).textContent = content;
    }
