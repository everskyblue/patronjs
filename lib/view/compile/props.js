    ...
    Object.keys(data).forEach(key => { 
        (function (key) {
            Object.defineProperty(newdata, key, {
                ...
                get: () => data[key],
                set: (newValue) => {
                    data[key] = newValue;console.log(newValue);
                    //store_elements = store_elements.filter(s => !!s);
                    /*var f = elms_compiled.filter(o => {
                        const map = o.a.find(c => c.scope === key);
                        if (map instanceof CompileSource) {
                            o.a.forEach(c => c.value(newdata));
                            return o;
                        }
                    })
                    console.log(f);*/
...
function on_bind(key_data) {
    ...
    function v_bind(node, data) {
        ...
        /** @since 0.4.0 */
        return new CompileSource(scope.called, (newValue) => {
            node.attributes.setNamedItem(attr);
            each(node, entry(newValue, each))
        });
    }
...
function on_for(source) {
    ...
    function v_for(node, data, each) {
        ...
        elms_compiled = copy;
        /** @since 0.4.0 */
        return new CompileSource((c3[2]), function (newdata) {
            del_nodes.forEach(elm_del => elm_del.remove());
            node.attributes.setNamedItem(attr);
        });
    }
...
function on_event(arr_events, callable) {
    ...
    function v_event(elm, data) {
        ...
        return new CompileSource(scope.called, (newdata) => {
...
function on_if(source) {
    ...
    function v_if(node, d) {
        ...
        console.log(node);
        return new CompileSource(scope.called, () => {
        });
    }
