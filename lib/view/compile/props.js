...
import { map_bind } from "./fun-map.js";
...
function on_bind(key_data) {
    ...
    function v_bind(node, data) {
        ...
        /** @since 0.4.0 */
        storeElement(scope.called, map_bind.bind(null, function () {
            console.log(node);
        }));
...
function on_for(source) {
    ...
    function v_for(node, data, each) {
        ...
        storeElement(c3[2], function () {
            node.setAttribute('@for', source);
        });
        /** @since 0.4.0 */
...
function on_event(arr_events, callable) {
    let scope = scope_code(callable),invoke, name_attr = `@event:${arr_events.join(':')}`;
    ...
    function v_event(elm, data) {
        ...
        storeElement(scope.called, function () {
            elm.setAttribute(name_attr, callable);
        });
        (elm.attributes.getNamedItem(name_attr)&&elm.attributes.removeNamedItem(name_attr))
    }
...
function on_if(source) {
    ...
    function v_if(node, d) {
        ...
        storeElement(scope.called, function () {
            ...
            node.setAttribute('@if', source);
        });

        (node.attributes.getNamedItem('@if')&&node.attributes.removeNamedItem('@if'));
    }
